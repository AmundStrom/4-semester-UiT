...
int main(void) {
	...
	char ps_buf[sizeof(struct pcb_helper_t) * 50]; /* ps information buffer, allocated for 50 processes */
	struct pcb_helper_t *ps_res = (struct pcb_helper_t *)ps_buf; /* parse ps buffer */
	int ps_amount; /* number of active processes */
	...
	while (1) {
		...
		/* Close shell */
			...
			if (argc == 1) {
				...
				ps_cmd(ps_buf, &ps_amount);

				/* Typecast to struct, parsing the bytes correct */
				ps_res = (struct pcb_helper_t *)ps_buf;

				for(int i = 0; i < ps_amount; i++){
					shprintf("pid: %d, status: %d\n", ps_res->pid, ps_res->status);
					ps_res++;
				}
