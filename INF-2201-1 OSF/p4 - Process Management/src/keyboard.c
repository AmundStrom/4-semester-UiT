...
/* allocate memory for one message with a body of 5 bytes */
char m_space[CHAR_MSG_SIZE];
msg_t *m = (msg_t *)m_space;
...
{
    // Get number of bytes available in the mailbox (space)
    mbox_stat(mbox, (int *)&count, (int *)&space);

    /* If size of character is larger then available bytes, return */
    if(space < sizeof(c->character)){
        return;
    }

    // Place character into message body
    m->body[0] = c->character;
    // size of message is size of character in bytes
    m->size = sizeof(c->character);
