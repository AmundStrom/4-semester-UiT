 ...
 * along with the text s. Then hang the os. Processes should use this
 * macro.
 */
#define ASSERT2(p, s)                                                          \
	do {                                                                   \
...
/*
 * Gives us the source file and line number where we decide to hang
 * the os.
 */
 ...
 * To use a message with a body of 50 bytes we must first allocate
 * space for
 * the message:
 *      char space[sizeof(int) + 50];
 * Then we declares a msg_t * variable that points to the allocated
 * space:
 *  msg_t *m = (msg_t *) space;
 * We can now access the size variable:
 *  m->size (at memory location &(space[0]) )
 * And the 15th character in the message:
 *      m->body[14] (at memory location &(space[0]) +
 *  sizeof(int) + 14 * sizeof(char))
 */
typedef struct {
...
/*
 * Structure used for interpreting the inforamtion
 * gained by the ps command.
 */
struct pcb_helper_t {
