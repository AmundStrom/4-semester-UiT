...
#define TRACE(s)                                                               \
	do {                                                                   \
		scrprintf(critical_count % 10 + 2, 0, "%d",                    \
			  critical_count / 10);                                \
		...
		scrprintf(critical_count % 10 + 2, 50, "%d",                   \
			  current_running->pid);                               \
...
#define RETURN_FROM_PROCEDURE asm volatile("ret")

/* Execute an 'iret' instruction */
#define RETURN_FROM_INTERRUPT asm volatile("iret")

/* Push x on the stack */
#define PUSH(x) asm volatile("pushl %0" ::"q"(x))

/* Pop something from the stack and save it in x */
#define POP(x) asm volatile("popl %0" : "=q"(x))
 ...
 * prior to disabling the interrupts.  The returned value may be
 * used as an argument to STI_FL() to enable the interrupts only if
 * the interrupts was enabled prior to invoking CLI_FL().
 */
#define CLI_FL()                                                               \
	do {                                                                   \
		long eflags;                                                   \
		__asm__ volatile(                                              \
			"pushfl  # Store EFlags on stack         \n"           \
...
#define STI_FL(eflags)                                                         \
	__asm__ volatile(                                                      \
		"btl $9,%0 # Check IF bit in stored EFlags \n"                 \
