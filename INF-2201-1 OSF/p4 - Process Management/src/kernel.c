...
void _start(void) {
	...
	init_syscall(SYSCALL_PS, (syscall_t)ps_cmd);
...
{
	char buffer[SECTOR_SIZE];	/* temp buffer */
	int os_size;				/* size of OS in sectors */

	/* Read 1 block starting at block 0, 
	 * copy read data into temp buffer.
	 * The data is the bootblock. */
	scsi_read(0, 1, buffer); // get bootblock

	/* Read 2 bytes starting at byte 2 into the buffer(bootblock), 
	 * copy read data into os_size variable.
	 * The data is the size of the OS in sectors. */
	bcopy(buffer + 2, (char *)&os_size, 2); // get size of OS in sectors

	/* Read 1 block starting at block "OS size" + 1 (so we skip shell), 
	 * copy read data into buffer.
	 * The data is the Process directory  */
	scsi_read(os_size + 1, 1, buffer); // get Process directory

	/* Read SECTOR_SIZE bytes into the buffer(Process directory),
	 * copy read data into buf.
	 * The data is the Process directory */
	bcopy(buffer, buf, SECTOR_SIZE); // copy Process directory to user provided buf
...
{
	/* Memory for code/data + stack size */
	uint32_t mem_size = (SECTOR_SIZE * size) + STACK_SIZE;

	/* Allocate memory */
	uint32_t proc = alloc_memory(mem_size);

	/* Read "size" blocks starting at block "location" from disk,
	 * copy data into "proc".
	 * The data is the code/data for the process. */
	scsi_read(location, size, (char *)proc);

	create_process(proc, mem_size);

	return 0;
}

/* View all the processes along with their pids, statuses.. 
 * buf: contains the inforamation
 * amount: contains the number of processes */
void ps_cmd(char *buf, int *amount)
{
	pcb_t *tmp_cur = current_running;
	int count = 0, loc = 0;
	uint32_t *tmp_val;

	do{
		/* if it is a thread, skip */
		if(tmp_cur->is_thread == TRUE){
			tmp_cur = tmp_cur->next;
			continue;
		}

		tmp_val = &tmp_cur->pid;
		bcopy((char *)tmp_val, buf + loc, sizeof(tmp_cur->pid)); // write the pid into the user provided buf
		loc += sizeof(tmp_cur->pid); // next available space

		tmp_val = &tmp_cur->status;
		bcopy((char *)tmp_val, buf + loc, sizeof(tmp_cur->status)); // write the status into the user provided buf
		loc += sizeof(tmp_cur->status); // next available space
		
		count++;
		tmp_cur = tmp_cur->next; // next job
	} while (tmp_cur != current_running);
