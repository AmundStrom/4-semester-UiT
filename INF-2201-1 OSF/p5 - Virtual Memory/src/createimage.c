...
static void create_images(int nfiles, char *files[]) {
	...
	if (options.vm == 1) {
		...
		/* if with vm, the os size will be only the size of kernel, if any */
		write_os_size(&image);
	...
	/* Padding up until sector 320, start of swap-space */
	while(image.nbytes != 320 * SECTOR_SIZE) {
		fputc(0, image.img);
		image.nbytes++;
	}
	printf("Padding up until sector: %d\n", image.nbytes / SECTOR_SIZE);

	/* Create swap space for vm, enough for 20 pages */
	int swap_space_bytes = 4096 * 20;
	int swap_space_sectors = swap_space_bytes / SECTOR_SIZE;
	for(int i = 0; i < swap_space_bytes; i++){
		fputc(0, image.img);
		image.nbytes++;
	}
	printf("Swap-space starts at sector: %d, and spans for %d sectors\n", 
		  (image.nbytes / SECTOR_SIZE) - swap_space_sectors, swap_space_sectors);
